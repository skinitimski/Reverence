        ...
        public override void ControlHandle(Key k)
        {
            switch (k)
            {
                ...
                case Key.Circle:
                    ...
                    {
                        if (Seven.Party.Inventory.UseItem(Seven.MenuState.ItemList.Option))
                        {
                            if (Seven.Party.Inventory.GetCount(Seven.MenuState.ItemList.Option) == 0)
                            {
                                Seven.MenuState.ItemScreen.ChangeControl(Seven.MenuState.ItemList);
                            }
                        }
            ...
            switch (option)
            {
                case 0:
                    cy = y0 - 8;
                    //Item.Target = Seven.Party[0];
                    break;
                case 1: 
                    cy = y0 - 8 + (y1 - y0); 
                    //Item.Target = Seven.Party[1]; 
                    break;
                case 2: 
                    cy = y0 - 8 + (y2 - y0); 
                    //Item.Target = Seven.Party[2]; 
                    break;
                default:
                    break;
            }
